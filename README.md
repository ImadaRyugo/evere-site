...
`/{lang}/blog/`（一覧）と `/{lang}/blog/{slug}/`（記事）。SEO目的の同一ドメイン・
サブディレクトリ運用（被リンク評価を evereapp.com 本体に蓄積させるため、
サブドメインや外部ブログサービスには置かない）。

- 記事本体: `src/content/blog/{lang}/{slug}.md`（同じ slug の言語同士が hreflang で相互リンク）
- frontmatter: `title` / `description` / `pubDate`（+ 任意で `updatedDate`, `tags`）
- 文言（一覧見出し・CTA等）: `src/i18n/blog.ts`
- スタイル: `src/styles/lp.css` の「ブログ」セクション

### 公開キュー（1日1本の自動公開）

公開待ち記事は `content-queue/YYYY-MM-DD-slug/{lang}.md` に置く（ビルド対象外）。
`scripts/publish-queue.mjs` が1日1回、キュー先頭の1エントリを
`src/content/blog/` へ移動し `pubDate` を当日に更新して commit & push する。
詳細は `content-queue/README.md` を参照。

```bash
# 動作確認（ファイル移動・commitなし）
node scripts/publish-queue.mjs --dry-run

# 手動で1本公開（移動 + pubDate更新 + commit + push）
node scripts/publish-queue.mjs
```
